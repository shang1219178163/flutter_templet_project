...
class SinglePickerPage extends StatefulWidget {
  const SinglePickerPage({super.key});
...
class _SinglePickerPageState extends State<SinglePickerPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('单列选择器')),
  ...
  void _onClickItem(var data, var selectData, {String? label}) {
    Pickers.showSinglePicker(
      ...
      onConfirm: (p, position) {
        ...
        debugPrint('longer >>> 返回数据类型：${p.runtimeType}');
        setState(() {
  ...
  void _onClickItem2() {
    ...
    Widget title =
        Center(child: NText('身高选择器', color: Colors.white, fontSize: 14));
