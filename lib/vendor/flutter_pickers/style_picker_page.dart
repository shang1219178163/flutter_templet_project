...
class _StylePickerPageState extends State<StylePickerPage> {
  ...
  void _onClickItem(pickerStyle) {
    Pickers.showSinglePicker(context,
        ...
        onConfirm: (p, __) => debugPrint('返回数据：$p'),
        onChanged: (p, __) => debugPrint('数据发生改变：$p'));
  }
  ...
  PickerStyle customizeStyle() {
    ...
    Widget _headMenuView = Container(
        ...
        height: menuHeight,
        child: Center(child: NText('净身高', color: Colors.white)));
    ...
    Widget _cancelButton = Container(
      ...
      child: NText('close', color: Colors.amberAccent, fontSize: 14),
    );
    ...
    Widget _commitButton = Container(
      ...
      child: NText('confirm', color: Colors.white, fontSize: 14),
    );
    ...
    Widget title =
        Center(child: NText('身高选择器', color: Colors.cyanAccent, fontSize: 14));
