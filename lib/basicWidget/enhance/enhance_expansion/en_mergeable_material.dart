...
class EnMergeableMaterial extends StatefulWidget {
  /// Creates a mergeable Material list of items.
  const EnMergeableMaterial({
    super.key,
  ...
  /// The children of the [EnMergeableMaterial].
  ...
  @override
  State<EnMergeableMaterial> createState() => _EnMergeableMaterialState();
}
...
class _EnMergeableMaterialState extends State<EnMergeableMaterial> with TickerProviderStateMixin {
  ...
  void _removeEmptyGaps() {
    for (var j = _children.length - 1; j >= 0; j -= 1) {
  ...
  @override
  void didUpdateWidget(EnMergeableMaterial oldWidget) {
    ...
    final oldOnly = oldKeys.difference(newKeys);
    ...
    while (i < newChildren.length && j < _children.length) {
      if (newOnly.contains(newChildren[i].key) || oldOnly.contains(_children[j].key)) {
        ...
        final newLength = i - startNew;
        final oldLength = j - startOld;
        ...
        if (newLength > 0) {
          if (oldLength > 1 || oldLength == 1 && _children[startOld] is MaterialSlice) {
            if (newLength == 1 && newChildren[startNew] is MaterialGap) {
              ...
              while (startOld < j) {
                final child = _children[startOld];
                if (child is MaterialGap) {
                  final gap = child;
                  gapSizeSum += gap.size;
                }
            ...
            if (newLength == 1 &&
              ...
              final gapSize = _getGapSize(startOld);
            ...
            // Grow gaps.
            for (var k = 0; k < newLength; k += 1) {
              final newChild = newChildren[startNew + k];
  ...
  BorderRadius _borderRadius(int index, bool start, bool end) {
    ...
    final cardRadius = widget.radius ?? kMaterialEdges[MaterialType.card]!.topLeft;

    var startRadius = Radius.zero;
    var endRadius = Radius.zero;
  ...
  Widget build(BuildContext context) {
    ...
    for (i = 0; i < _children.length; i += 1) {
      if (_children[i] is MaterialGap) {
        ...
        var child = slice.child;

        if (widget.hasDividers) {
          final hasTopDivider = _willNeedDivider(i - 1);
          final hasBottomDivider = _willNeedDivider(i + 1);

          final divider = Divider.createBorderSide(
            context,
          ...
          child = AnimatedContainer(
            key: _CustomMergeableMaterialSliceKey(_children[i].key),
    ...
    return _CustomMergeableMaterialListBody(
      mainAxis: widget.mainAxis,
...
class _CustomMergeableMaterialSliceKey extends GlobalKey {
  const _CustomMergeableMaterialSliceKey(this.value) : super.constructor();

  final LocalKey value;

  @override
  bool operator ==(Object other) {
    return other is _CustomMergeableMaterialSliceKey && other.value == value;
  }
  ...
  @override
  String toString() {
    return '_CustomMergeableMaterialSliceKey($value)';
  }
}

class _CustomMergeableMaterialListBody extends ListBody {
  const _CustomMergeableMaterialListBody({
  ...
  RenderListBody createRenderObject(BuildContext context) {
    return _RenderCustomMergeableMaterialListBody(
...
class _RenderCustomMergeableMaterialListBody extends RenderListBody {
  _RenderCustomMergeableMaterialListBody({
    super.axisDirection,
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    while (child != null) {
      ...
      final rect = (childParentData.offset + offset) & child.size;
      if (index.isEven) {
