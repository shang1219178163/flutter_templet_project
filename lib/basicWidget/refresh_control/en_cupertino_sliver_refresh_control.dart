...
class _EnCupertinoSliverRefresh extends SingleChildRenderObjectWidget {
  const _EnCupertinoSliverRefresh({
  ...
  // _RenderEnCupertinoSliverRefresh will paint the child in the available
  // space either way but this instructs the _RenderEnCupertinoSliverRefresh
  // on whether to also occupy any layoutExtent space or not.
  final bool hasLayoutExtent;

  @override
  _RenderEnCupertinoSliverRefresh createRenderObject(BuildContext context) {
    return _RenderEnCupertinoSliverRefresh(
  ...
  @override
  void updateRenderObject(BuildContext context, covariant _RenderEnCupertinoSliverRefresh renderObject) {
    renderObject
...
class _RenderEnCupertinoSliverRefresh extends RenderSliver with RenderObjectWithChildMixin<RenderBox> {
  _RenderEnCupertinoSliverRefresh({
...
/// the same values passed into the [EnCupertinoSliverRefreshControl].
///
...
typedef EnRefreshControlIndicatorBuilder = Widget Function(
  BuildContext context,
  EnRefreshIndicatorMode refreshState,
  double pulledExtent,
  double refreshTriggerPullDistance,
  double refreshIndicatorExtent,
);

/// A callback function that's invoked when the [EnCupertinoSliverRefreshControl] is
/// pulled a `refreshTriggerPullDistance`. Must return a [Future]. Upon
/// completion of the [Future], the [EnCupertinoSliverRefreshControl] enters the
/// [EnRefreshIndicatorMode.done] state and will start to go away.
typedef EnRefreshCallback = Future<void> Function();
...
///
/// The [builder] function will be informed of the current [EnRefreshIndicatorMode]
/// when invoking it, except in the [EnRefreshIndicatorMode.inactive] state when
...
///    [EnCupertinoSliverRefreshControl] is part of the scrollable and actively occupies
///    scrollable space.
class EnCupertinoSliverRefreshControl extends StatefulWidget {
  ...
  /// a refresh.
  const EnCupertinoSliverRefreshControl({
    super.key,
    this.refreshTriggerPullDistance = _defaultRefreshTriggerPullDistance,
    this.refreshIndicatorExtent = _defaultRefreshIndicatorExtent,
    this.builder = buildEnRefreshIndicator,
    this.onRefresh,
  ...
  /// null and the [builder] will build in the [EnRefreshIndicatorMode.armed] state.
  ...
  /// overscroll.
  final EnRefreshControlIndicatorBuilder? builder;
  ...
  /// [EnRefreshIndicatorMode.refresh] state until the [Future] completes.
  ///
  /// Can be null, in which case a single frame of [EnRefreshIndicatorMode.armed]
  /// state will be drawn before going immediately to the [EnRefreshIndicatorMode.done]
  ...
  /// Retrieve the current state of the EnCupertinoSliverRefreshControl. The same as the
  ...
  static Widget buildEnRefreshIndicator(
    BuildContext context,
    EnRefreshIndicatorMode refreshState,
    double pulledExtent,
    double refreshTriggerPullDistance,
  ...
  static Widget _buildIndicatorForRefreshState(
      EnRefreshIndicatorMode refreshState, double radius, double percentageComplete) {
    switch (refreshState) {
      case EnRefreshIndicatorMode.drag:
      ...
      case EnRefreshIndicatorMode.armed:
      case EnRefreshIndicatorMode.refresh:
        // Once we're armed or performing the refresh, we just show the normal spinner.
        return CupertinoActivityIndicator(radius: radius);
      case EnRefreshIndicatorMode.done:
        // When the user lets go, the standard transition is to shrink the spinner.
        return CupertinoActivityIndicator(radius: radius * percentageComplete);
      case EnRefreshIndicatorMode.inactive:
...
class _EnCupertinoSliverRefreshControlState extends State<EnCupertinoSliverRefreshControl> {
  ...
  late EnRefreshIndicatorMode refreshState;
  ...
  void initState() {
    super.initState();
    refreshState = EnRefreshIndicatorMode.inactive;
  }

  // A state machine transition calculator. Multiple states can be transitioned
  // through per single call.
  EnRefreshIndicatorMode transitionNextState() {
    EnRefreshIndicatorMode nextState;

    void goToDone() {
      nextState = EnRefreshIndicatorMode.done;
    ...
    switch (refreshState) {
      case EnRefreshIndicatorMode.inactive:
        if (latestIndicatorBoxExtent <= 0) {
          return EnRefreshIndicatorMode.inactive;
        } else {
          nextState = EnRefreshIndicatorMode.drag;
        }
        continue drag;
      drag:
      case EnRefreshIndicatorMode.drag:
        if (latestIndicatorBoxExtent == 0) {
          ...
          return EnRefreshIndicatorMode.drag;
        } else {
          ...
          return EnRefreshIndicatorMode.armed;
        }
      case EnRefreshIndicatorMode.armed:
        if (refreshState == EnRefreshIndicatorMode.armed && refreshTask == null) {
          goToDone();
          continue done;
        }
        ...
        if (latestIndicatorBoxExtent > widget.refreshIndicatorExtent) {
          ...
          nextState = EnRefreshIndicatorMode.refresh;
        }
        continue refresh;
      refresh:
      case EnRefreshIndicatorMode.refresh:
        if (refreshTask != null) {
          return EnRefreshIndicatorMode.refresh;
        } else {
          goToDone();
        }
        continue done;
      ...
      case EnRefreshIndicatorMode.done:
        ...
        if (latestIndicatorBoxExtent > widget.refreshTriggerPullDistance * _inactiveResetOverscrollFraction) {
          return EnRefreshIndicatorMode.done;
        } else {
          nextState = EnRefreshIndicatorMode.inactive;
        }
    }
  ...
  Widget build(BuildContext context) {
    return _EnCupertinoSliverRefresh(
