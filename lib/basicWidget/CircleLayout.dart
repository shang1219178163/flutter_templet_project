...
class _RingDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    /// 容器半径参考值
    var fatherRadius = min(size.width, size.height) / 2;

    var childRadius = _getChildRadius(fatherRadius, 360 / count);

    var constraintsSize = Size(childRadius * 2, childRadius * 2);

    /// 遍历child获取他们所需的空间,得到最宽child的宽度以及最高child的高度
    /// 用来计算一个可用半径r
    /// r = 父容器给定的半径 - 最大子部件的"半径"
    var sizeCache = <Size>[];
    var largersRadius = 0.0;
    for (var i = 0; i < count; i++) {
      if (!hasChild(i)) {
        break;
      }

      var childSize = layoutChild(i, BoxConstraints.loose(constraintsSize));
      // 缓存所有子部件尺寸 备用
      sizeCache.add(Size.copy(childSize));

      var _radius = max(childSize.width, childSize.height) / 2;
    ...
    for (var i = 0; i < count; i++) {
      ...
      var offset = _getChildCenterOffset(
      ...
      var cr = max(sizeCache[i].width, sizeCache[i].height) / 2;
      offset -= Offset(cr, cr);
...
Offset _getChildCenterOffset({
  ...
  /// x = a + r * cos(radian)
  /// y = b + r * sin(radian)
  var radian = _radian(360 / count);
  var radianOffset = _radian(360 + initAngle * direction);
  var x = centerPoint.dx + radius * cos(radian * which + radianOffset);
  var y = centerPoint.dy + radius * sin(radian * which + radianOffset);
  return Offset(x, y);
}
