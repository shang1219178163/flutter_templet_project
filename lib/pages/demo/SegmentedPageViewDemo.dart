...
class _SegmentedPageViewDemoState extends State<SegmentedPageViewDemo> {
  ...
  final items = <({String title, String icon, Widget child,})>[
  (
  title: "医生",
  ...
  late final selectedItemVN = ValueNotifier(items[0]);
  ...
  buildSlidingSegmentedControl({
    ...
    return Column(
      ...
      children: [
        ValueListenableBuilder<({String title, String icon, Widget child,})>(
          ...
          builder: (context, selectedItem, child){
            ...
            return segmentedBuilder?.call(segmentedControl) ?? Container(
              // padding: EdgeInsets.symmetric(horizontal: 48, vertical: 14),
              padding: EdgeInsets.only(left: 48, right: 48, top: 12, bottom: 16),
              decoration: BoxDecoration(
                // color: Colors.green,
  ...
  Widget buildPageView() {
    return PageView(
      ...
      onPageChanged: (index) {
        selectedItemVN.value = items[index];
      },
