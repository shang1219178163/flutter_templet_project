...
class GlobalIsolate {
  ...
  static Future _initNewIsolate() async {
    ...
    _newIsolate = await Isolate.spawn(_createNewIsolateContext, rootIsolateSendPort);
  ...
  static Future<void> _createNewIsolateContext(SendPort rootIsolateSendPort) async {
    ...
    newIsolateReceivePort.listen((message) {
      if (message is SendMessage) {
        message
            .work(message.params)
            .then((value) => rootIsolateSendPort.send(ReceiveMessage(id: message.id, result: value)));
      }
    });
  }
}
